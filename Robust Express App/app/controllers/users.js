...
module.exports.controller_1 = function(app){
  ...
  userRouter.get('/forgot',function(req,res){
    res.render('forgotPass',{user:req.session.user});
  });
  ...
  userRouter.post('/email',function(req,res){
    userModel.findOne({$and:[{'email':req.body.email}]},function(err,foundUser){
      ...
      let mailOptions = {
         from: '" Shoppers-Point 👻" <ajay.jadhav.akj@gmail.com>', // sender address
         to: req.body.email, // list of receivers ${req.body.email}
         subject: ' Password For Shopper-Point Login', // Subject line
         text: 'your password is : req.password' , // plain text body
         html: ' Click On This Link To Change The Password : http://localhost:1993/linkyfy'    // html body
      };
  ...
  userRouter.post('/send',function(req,res){
    ...
    let transporter = nodemailer.createTransport({
       ...
       secure: false, // true for 465, false for other ports
       auth: {
           user: 'ajay.jadhav.akj@gmail.com', // generated ethereal user
           pass: 'ajay@9545534349'  // generated ethereal password
       },
       tls:{
         rejectUnauthorized:false
       }
  ...
  userRouter.post('/link',function(req,res){
    userModel.findOne({$and:[{'email':req.body.email}]},function(err,foundUser){
      ...
      //    var myResponse =responseGenerator.generate(false,"Login Successfully",200,foundUser);
        ...
        req.session.user = foundUser;
        delete req.session.user.password;
        res.render('changePassword_1',{user:req.session.user, cart:req.session.cart});
      }
    }); //end find
  });// end login api
  app.use('/', userRouter);
